    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-xl border p-5 shadow-sm">
          <h2 className="text-sm font-semibold mb-4 flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-warning" /> {t("dashboard.hinweise")}</h2>
          <div className="space-y-2.5">
            {fahrtenOhnePreis.length > 0 && <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-lg text-xs font-medium text-amber-700 dark:text-amber-300">{t("dashboard.fahrtenOhnePreis", { count: fahrtenOhnePreis.length })}</div>}
            {fzOhneKosten.length > 0 && <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-lg text-xs font-medium text-amber-700 dark:text-amber-300">{t("dashboard.fahrzeugeOhneKosten", { count: fzOhneKosten.length })}</div>}
            {fahrtenOhnePreis.length === 0 && fzOhneKosten.length === 0 && <p className="text-muted-foreground text-sm">{t("dashboard.allesOk")}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card rounded-xl border p-5 shadow-sm">
          ...
          <div className="space-y-1">
            {fahrer.filter(f => f.status === "aktiv").map((f, i) => {
