...
const ALL_COLUMNS: ColumnDef[] = [
  {
    key: "kennzeichen", label: "Kennzeichen", fixed: true,
    render: (d) => <span className="font-mono text-sm font-medium">{d.kennzeichen}</span>,
    className: "px-4 py-3", headerClassName: "text-left px-4 py-3",
  },
  {
    key: "modell", label: "Fahrzeugmodell",
    render: (d) => <span className="text-sm">{d.marke} {d.modell}</span>,
    className: "px-4 py-3", headerClassName: "text-left px-4 py-3",
  },
  {
  ...
  },
  {
    ...
    render: (d) => <span className="text-sm tabular-nums">{d.fahrtenCount}</span>,
    className: "px-4 py-3 text-right", headerClassName: "text-right px-4 py-3",
  },
  {
    key: "einnahmen", label: "Einnahmen", sortKey: "einnahmen",
    render: (d) => <span className="text-sm tabular-nums">{formatCurrency(d.einnahmen)}</span>,
    className: "px-4 py-3 text-right", headerClassName: "text-right px-4 py-3 cursor-pointer",
  },
  {
    key: "kosten", label: "Kosten", sortKey: "kosten",
    render: (d) => <span className="text-sm tabular-nums">{formatCurrency(d.kosten)}</span>,
    className: "px-4 py-3 text-right", headerClassName: "text-right px-4 py-3 cursor-pointer",
  },
  {
    key: "ergebnis", label: "Ergebnis", sortKey: "ergebnis",
    render: (d) => <span className={cn("text-sm font-medium tabular-nums", d.ergebnis >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-destructive")}>{formatCurrency(d.ergebnis)}</span>,
    className: "px-4 py-3 text-right", headerClassName: "text-right px-4 py-3 cursor-pointer",
  },
  {
    key: "marge", label: "Marge", sortKey: "marge",
    render: (d) => <span className="text-sm tabular-nums">{d.marge.toFixed(1)} %</span>,
    className: "px-4 py-3 text-right", headerClassName: "text-right px-4 py-3 cursor-pointer",
  },
  {
    key: "baujahr", label: "Baujahr",
    render: (d) => <span className="text-sm tabular-nums">{d.baujahr}</span>,
    className: "px-4 py-3", headerClassName: "text-left px-4 py-3",
  },
  {
    key: "farbe", label: "Farbe",
    render: (d) => <span className="text-sm">{d.farbe}</span>,
    className: "px-4 py-3", headerClassName: "text-left px-4 py-3",
  },
];
...
function loadConfig() {
  try {
    ...
    if (saved) {
      ...
      const safeOrder = ["kennzeichen", ...order.filter((k: string) => k !== "kennzeichen")];
      return { visible: [...new Set(["kennzeichen", ...visible])] as string[], order: safeOrder as string[] };
    }
  } catch {}
...
export default function FahrzeugeListe() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(d =>
        d.kennzeichen.toLowerCase().includes(s) ||
        d.marke.toLowerCase().includes(s) ||
        d.modell.toLowerCase().includes(s) ||
        d.farbe.toLowerCase().includes(s)
      );
    }
    if (sortKey) {
      r.sort((a, b) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-card rounded-xl border shadow-sm overflow-hidden">
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id} className="border-b last:border-0 hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => navigate(`/fahrzeuge/${d.id}`)}>
                  {visibleColumns.map(col => (
                    <td key={col.key} className={col.className}>{col.render(d)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        {filtered.length === 0 && (
          <div className="p-12 text-center text-muted-foreground text-sm">Keine Fahrzeuge gefunden.</div>
        )}
      </div>
    </div>
  );
