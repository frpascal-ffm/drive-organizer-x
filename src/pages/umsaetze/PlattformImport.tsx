...
import { fahrerList, fahrzeuge, formatCurrency, formatDate } from "@/data/mockData";
import { ArrowLeft, ArrowRight, Upload, Clipboard, X, Check, Image, FileText, CalendarIcon } from "lucide-react";
...
export default function PlattformImport() {
  ...
  const netto = betrag && provision
    ? parseFloat(betrag) - parseFloat(provision || "0")
    : parseFloat(betrag || "0");

  const selectedFahrer = fahrerList.find(f => f.id === fahrerId);
  const selectedFahrzeug = fahrzeuge.find(f => f.id === fahrzeugId);

  return (
    <div className="space-y-6 animate-fade-in max-w-3xl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/umsaetze")} className="shrink-0">
          ...
          >
            ...
            {file ? (
              <div className="space-y-3">
                {preview ? (
                  <img src={preview} alt="Vorschau" className="max-h-48 mx-auto rounded-lg shadow-sm" />
                ) : (
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">CSV, Excel, PDF oder Screenshot</p>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !vonDatum && "text-muted-foreground")}>
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {vonDatum ? format(vonDatum, "dd.MM.yyyy") : "Datum wählen"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar mode="single" selected={vonDatum} onSelect={setVonDatum} locale={de} className="p-3 pointer-events-auto" />
                </PopoverContent>
              </Popover>
            </div>
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !bisDatum && "text-muted-foreground")}>
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {bisDatum ? format(bisDatum, "dd.MM.yyyy") : "Datum wählen"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar mode="single" selected={bisDatum} onSelect={setBisDatum} locale={de} className="p-3 pointer-events-auto" />
                </PopoverContent>
              </Popover>
            </div>
          </div>
      ...
      {step === 2 && (
        <div className="bg-card rounded-xl border p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              <label className="text-sm font-medium">Bruttoumsatz (€)</label>
              <Input type="number" min="0" step="0.01" placeholder="0,00" value={betrag} onChange={e => setBetrag(e.target.value)} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Provision (€)</label>
              <Input type="number" min="0" step="0.01" placeholder="0,00" value={provision} onChange={e => setProvision(e.target.value)} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Anzahl Fahrten</label>
              <Input type="number" min="0" placeholder="0" value={fahrtenAnzahl} onChange={e => setFahrtenAnzahl(e.target.value)} />
            </div>
          </div>
          {betrag && (
            <div className="bg-muted/30 rounded-lg p-4">
              <p className="text-xs text-muted-foreground mb-1">Nettoumsatz</p>
              <p className="text-xl font-semibold text-primary">{formatCurrency(netto)}</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          {preview && (
            <div className="bg-card rounded-xl border p-4">
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-3">Hochgeladene Datei</p>
              <img src={preview} alt="Screenshot" className="max-h-40 rounded-lg shadow-sm" />
            </div>
          )}
          <div className="bg-card rounded-xl border divide-y">
            ...
            <div className="px-5 py-3 flex justify-between">
              ...
              <span className="text-sm font-medium">{vonDatum ? format(vonDatum, "dd.MM.yyyy") : "–"} – {bisDatum ? format(bisDatum, "dd.MM.yyyy") : "–"}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Fahrer</span>
              <span className="text-sm font-medium">{selectedFahrer ? `${selectedFahrer.vorname} ${selectedFahrer.nachname}` : "–"}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Fahrzeug</span>
              <span className="text-sm font-medium">{selectedFahrzeug?.kennzeichen || "–"}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Bruttoumsatz</span>
              <span className="text-sm font-medium">{formatCurrency(parseFloat(betrag || "0"))}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Provision</span>
              <span className="text-sm font-medium">{formatCurrency(parseFloat(provision || "0"))}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Nettoumsatz</span>
              <span className="text-sm font-semibold text-primary">{formatCurrency(netto)}</span>
            </div>
            <div className="px-5 py-3 flex justify-between">
              <span className="text-sm text-muted-foreground">Fahrten</span>
              <span className="text-sm font-medium">{fahrtenAnzahl || "–"}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="bg-card rounded-xl border p-12 text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Der Plattformumsatz von <span className="font-medium text-foreground">{formatCurrency(netto)}</span> ({plattform}) wurde erfolgreich erfasst.
          </p>
          <div className="flex gap-3 justify-center pt-2">
            <Button variant="outline" onClick={() => navigate("/umsaetze")}>Zurück zu Umsätze</Button>
            <Button onClick={() => {
              setStep(0); setFile(null); setPreview(null); setPlattform(""); setVonDatum(undefined); setBisDatum(undefined);
              setFahrerId(""); setFahrzeugId(""); setBetrag(""); setProvision(""); setFahrtenAnzahl(""); setDone(false);
        ...
        <div className="flex justify-between pt-2">
          ...
          {step < 3 ? (
            ...
            <Button disabled={importing} onClick={handleImport}>
              {importing ? "Importiere…" : "Import abschließen"}
